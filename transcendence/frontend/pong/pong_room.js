...
export class PongRoom {
  ...
  get availableSlots() {
    return (
      this._maxPlayers - this._players.length - this._pendingInvitations.length
    );
  }
  get state() {
    return this._state;
  ...
  updateOwner(value) {
    if (this._owner.id !== value.id) {
      logger.info(
        `Setting owner from ${this._owner.username} to ${value.username}`
      );
      this._owner = value;
  ...
  updatePlayers(value) {
    ...
    if (
      ...
      logger.info(
        `Setting players from ${JSON.stringify(
          this._players
  ...
  updatePendingInvitations(value) {
    if (JSON.stringify(this._pendingInvitations) !== JSON.stringify(value)) {
      logger.info(
        `Setting pending invitations from ${JSON.stringify(
          this._pendingInvitations
        )} to ${JSON.stringify(value)}`
      );
  ...
  updateFromState(roomState) {
    ...
    if (JSON.stringify(this._players) !== JSON.stringify(roomState.players)) {
      this._players = Array.isArray(roomState.players) ? roomState.players : [];
      hasChanged = true;
    }
    if (
      JSON.stringify(this._pendingInvitations) !==
      JSON.stringify(roomState.pending_invitations)
    ) {
      this._pendingInvitations = Array.isArray(roomState.pending_invitations)
        ? roomState.pending_invitations
        : [];
      hasChanged = true;
    }
